...
CLEAN_FILE   <- file.path("analytics", "outputs", "cleaned_food_prices.csv")
OUTPUT_DIR   <- file.path("analytics", "outputs")

if (!file.exists(CLEAN_FILE)) {
  stop("Run analytics/01_data_cleaning.R first.\nMissing: ", CLEAN_FILE)
}
...
write_csv(
  modelling_dataset %>% select(all_of(final_cols)),
  file.path(OUTPUT_DIR, "modelling_dataset.csv")
)
