...
for (out in c(OUTPUT_DIR, OUTPUT_REPORTS_DIR)) {
  write_csv(all_results, file.path(out, "statistical_results.csv"))
}
cat(sprintf("   Saved: %s\n", file.path(OUTPUT_REPORTS_DIR, "statistical_results.csv")))

# Plain-text report
write_report <- function(report_path) {
  sink(report_path)
...
write_report(file.path(OUTPUT_REPORTS_DIR, "statistical_report.txt"))
write_report(file.path(OUTPUT_DIR, "statistical_report.txt"))

cat(sprintf("   Report: %s\n", file.path(OUTPUT_REPORTS_DIR, "statistical_report.txt")))
cat("\n=== Phase 4 Complete ===\n")
cat("Next step: Rscript analytics/scripts/05_model_training.R\n")
