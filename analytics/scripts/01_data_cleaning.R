...
for (out in c(OUTPUT_DIR, OUTPUT_DATA_DIR)) {
  write_csv(cleaned_df, file.path(out, "cleaned_food_prices.csv"))
  write_csv(series_coverage, file.path(out, "series_coverage.csv"))
  write_csv(data_quality_summary, file.path(out, "data_quality_summary.csv"))
}
cat(sprintf("   Saved: %s\n", file.path(OUTPUT_DATA_DIR, "cleaned_food_prices.csv")))
cat(sprintf("   Saved: %s\n", file.path(OUTPUT_DATA_DIR, "series_coverage.csv")))
cat(sprintf("   Saved: %s\n", file.path(OUTPUT_DATA_DIR, "data_quality_summary.csv")))

cat("\n=== Phase 2 Complete ===\n")
cat("Next step: Rscript analytics/scripts/02_descriptive_analysis.R\n")
